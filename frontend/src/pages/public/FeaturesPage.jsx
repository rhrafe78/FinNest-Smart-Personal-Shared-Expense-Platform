...
export const FeaturesPage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-20 space-y-14">
      {/* Header */}
      <div className="text-center max-w-3xl mx-auto space-y-4">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-indigo-200 dark:border-brand-900 bg-indigo-50/90 dark:bg-brand-950/60 text-xs font-bold text-indigo-800 dark:text-brand-300 shadow-sm">
        ...
        <p className="text-base sm:text-lg text-slate-700 dark:text-slate-300 leading-relaxed">
          From your personal pocket money and monthly budgets to shared flat rent, groceries, and roommate debt settlement—explore all 12 core capabilities.
        </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
        {filteredFeatures.map((f, i) => {
          ...
          return (
            <div
              key={i}
              className={`p-7 rounded-3xl border-2 ${f.borderColor} bg-gradient-to-br ${f.lightBg} dark:from-slate-900/90 dark:via-slate-900 dark:to-slate-900 shadow-md hover:shadow-xl transition-all flex flex-col justify-between space-y-5`}
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className={`w-12 h-12 rounded-2xl ${f.iconBg} flex items-center justify-center shadow-md`}>
                    <Icon className="w-6 h-6" />
                  </div>
                  <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold ${f.badgeClass}`}>
                    {f.badge}
                  </span>
                </div>
              ...
              <div className="pt-2 border-t border-slate-200/60 dark:border-slate-800/80 flex items-center justify-between text-xs font-semibold text-slate-500 dark:text-slate-400">
      ...
      <div className="text-center pt-8">
        <Link to="/register">
          <Button variant="primary" size="lg" className="px-8 py-3.5 shadow-xl shadow-brand-500/25 font-bold text-base">
            Start Using All Features Free <ArrowRight className="w-5 h-5 ml-2" />
          </Button>
        </Link>
      </div>
    </div>
